// Made by Neonagee https://github.com/Neonagee/GUIStylesViewer
...
class GUIStylesViewer : EditorWindow
{
    ...
    public class Style
    {
        ...
        public bool pressed;
        public Style(GUIStyle Style, GUIContent Content, Vector2 Size, float Height, bool isIcon)
        {
            style = Style;
            content = Content;
            this.isIcon = isIcon;
    ...
    void OnEnable()
    {
        settingsCollapse = new AnimBool(Repaint) { speed = 2.5f };
        loaded = false;
    }

    GUIStyle regionBg;
    GUIStyle grayMiniLabel;
    GUIStyle optionsToggle;
    ...
    GUIStyle richLabel;
    //GUIStyle settingsIcon;
    void GetGUIStyles()
    {
        //settingsIcon = new GUIStyle("Button");
        //settingsIcon.normal.(Texture2D)EditorGUIUtility.IconContent("SettingsIcon").image;
        ...
        optionsToggle = new GUIStyle("PaneOptions") { margin = new RectOffset(0, 0, 2, 0) };
        Texture2D settingsIcon = (Texture2D)EditorGUIUtility.IconContent("SettingsIcon").image;
        optionsToggle.normal.background = settingsIcon;
    ...
    public static object CallPrivateFunction(System.Type type, string methodName, params object[] args)
    {
        var mi = type.GetMethod(methodName, BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Static);
        if (mi != null)
        {
            return mi.Invoke(type, args);
        }
        return null;
    }

    readonly string[] builtinIcons = new string[]
    {
        #region Editor Icons List
        "_Help",
        "_Popup",
    ...
    readonly struct EditorIcons
    {
        public readonly Style[] icons;
        public EditorIcons(in string[] names)
        {
            GUIStyle iconsStyle = new GUIStyle("Box");
            iconsStyle.normal.background = null;
            iconsStyle.hover.background = null;
            iconsStyle.focused.background = null;
            iconsStyle.onNormal.background = null;
            iconsStyle.onHover.background = null;
            iconsStyle.onFocused.background = null;

            Vector2 size = iconsStyle.CalcSize(GUIContent.none);
            float heightWithContent = iconsStyle.CalcHeight(new GUIContent(sampleText), 100);

            icons = new Style[names.Length];
            for (int i = 0; i < names.Length; i++)
            {
                GUIStyle icon = new GUIStyle(iconsStyle);
                Texture2D tex = (Texture2D)CallPrivateFunction(typeof(EditorGUIUtility), "LoadIcon", names[i]);
                ...
                icon.normal.background = tex;
                var style = new Style
                    (
                        icon,
                        new GUIContent(icon.normal.background.name),
                        size,
                        heightWithContent,
                        isIcon: true
                    );
        ...
        foreach (GUIStyle m_style in inspectorSkin)
        {
            var style = new Style
                (
                    ...
                    m_style.CalcHeight(new GUIContent(sampleText), customWidth), // Height With Content
                    isIcon: false
                );
            styles.Add(style);
        }
        // Load built-in icons
        editorIcons = new EditorIcons(in builtinIcons);
        styles.AddRange(editorIcons.icons);

        // Sort Styles list by name
        styles.Sort((x, y) => string.Compare(x.nameToLower, y.nameToLower));
    ...
    {
        for (int i = 0; i < filteredStyles.Count; i++)
        {
            if (filteredStyles[i].style == null)
                continue;
            float width = useCustomWidth ? customWidth : filteredStyles[i].isLabel ? position.width / 1.5f : filteredStyles[i].width;

            filteredStyles[i].heightWithContent = filteredStyles[i].style.CalcHeight(new GUIContent(sampleText), width);
        }
    }
    Vector2 scrollPos;
    ...
    void FilterStyles()
    {
        ...
        filteredStyles.Capacity = styles.Count;
        for (int i = 0; i < styles.Count; i++)
        {
            if (!styles[i].isLoaded)
                continue;
            if (styles[i].isIcon && !showIcons)
                continue;
    ...
    void OnGUI()
    {
        ...
        EditorGUILayout.LabelField("Count: " + filteredStyles.Count, grayMiniLabel, GUILayout.Width(80));

        EditorGUI.BeginChangeCheck();
        searchFilter = EditorGUILayout.TextField(searchFilter, searchTextField, GUILayout.MinWidth(10), GUILayout.ExpandWidth(true), GUILayout.MaxWidth(10000));
        ...
        // Options button
        Rect settingsIconRect = EditorGUILayout.BeginHorizontal();
        Matrix4x4 matrix = GUI.matrix;
        GUIUtility.RotateAroundPivot(Mathf.Lerp(0, 90, settingsCollapse.faded), settingsIconRect.position + new Vector2(settingsIconRect.width / 4 + 4, settingsIconRect.width / 4 + 4));
        settingsCollapse.target = GUILayout.Toggle(settingsCollapse.target, "", optionsToggle);
        ...
        if (EditorGUILayout.BeginFadeGroup(settingsCollapse.faded))
        {
            ...
            // Show icons toggle
            EditorGUI.BeginChangeCheck();
            showIcons = EditorGUILayout.Toggle("Show Icons", showIcons);
            if (EditorGUI.EndChangeCheck())
                filterChanged = true;
            ...
            optimizeScroll = GUILayout.Toggle(optimizeScroll, 
                new GUIContent(" Optimized Scroll"), 
        ...
        for (int i = scrollStart; i < scrollEnd; i++)
        {
            ...
            bool isIcon = filteredStyles[i].isIcon;
            ...
            try // Need try catch to disable stupid GUI errors
            {
                ...
                if (GUILayout.Button("<color=#" + labelHex + ">" + (i + 1) + "</color>  " + filteredStyles[i].content.text +
                    (isIcon || pressable ? "<color=#" + labelHex + "> " + (isIcon ? "Icon" : "Pressable") + "</color>" : ""), 
    ...
    {
        string savePath;
        if(!string.IsNullOrEmpty((savePath = EditorUtility.SaveFilePanel("Save GUI Texture", EditorApplication.applicationPath, 
            state.background.name, "png"))))
        {
